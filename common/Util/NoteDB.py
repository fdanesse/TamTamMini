...
class Page():
    ...
    def __init__(self, beats, color=0, instruments=False, local=True):
        # , tempo, insruments, color = 0 ):
        ...
        if not instruments:
            self.instruments = [
                self.instrumentDB.instNamed[
                "kalimba"].instrumentId for i in range(
                Config.NUMBER_OF_TRACKS-1)] + [
                self.instrumentDB.instNamed["drum1kit"].instrumentId]
...
class NoteDB():
    ...
    def duplicatePages(self, which, after=False):
        ...
        new = {}
        for cp in sorted:
            id = self._newPage(-1, Page(
                self.pages[cp].beats,
                self.pages[cp].color,
                self.pages[cp].instruments))
    ...
    def getInstruments(self, pages):
        ...
        for page in pages:
            ...
            for track in range(Config.NUMBER_OF_TRACKS):
                list.append(
                    self.instrumentDB.instId[
                    self.pages[page].instruments[track]].name)
   ...
   #-- private --------------------------------------------
    ...
    def addNote(self, nid, page, track, cs, hint=False):
        ...
        while at > 0:
            ...
            if onset <= cs.onset:
                ...
                elif self.noteS[page][track][at-1].cs.pitch <= cs.pitch:
                    break
        ...
        while at < last:
            ...
            if onset >= cs.onset:
                ...
                elif self.noteS[page][track][at].cs.pitch > cs.pitch:
                    break
        ...
        for par in self.parasiteList.keys():
            ...
            # give parasites the option of return something other than themselves
            self.parasiteD[page][track][par][nid] = parasite.attach()
            self.parasiteS[page][track][par].insert(at, parasite.attach())

        if hint:
            hint[0] = at + 1 # assume the next note will fall after this one
    ...
    def addNotes(self, stream):
        ...
        while p != -1:
            ...
            for j in range(N):
                new[p][t].append(self.addNote(
                -1, p, t, self._readstream(stream, i), hint))
    ...
    def duplicateNote(self, page, track, id, toPage, toTrack, offset):
        ...
        if cs.onset >= ticks:
            return False # off the end of the page
    ...
    def _resortNote(self, page, track, id):
        ...
        while ins > 0: # check backward
            ...
            if onset <= cs.onset:
                ...
                elif self.noteS[page][track][ins-1].cs.pitch <= cs.pitch:
                    break
        ...
        if ins == out: # check forward
            ...
            while ins < last:
                ...
                if onset >= cs.onset:
                    ...
                    elif self.noteS[page][track][ins].cs.pitch > cs.pitch:
                        break
    ...
    def notesToClipboard(self, stream):
        ...
        while p != -1:
            ...
            for j in range(N):
                ...
                if cs.onset < pageArea["limit"][0]:
                    pageArea["limit"][0] = cs.onset

                if cs.onset + cs.duration > pageArea["limit"][1]:
                    pageArea["limit"][1] = cs.onset + cs.duration
        ...
        for p in pages:
            ...
            for t in trackMap.keys():
                ...
                for n in self.noteS[p][t]:
                    ...
                    if area["limit"][0] <= start < area["limit"][1]:
                        start = area["limit"][1]

                    if area["limit"][0] < end <= area["limit"][1]:
                        end = area["limit"][0]

                    if start < area["limit"][0] and end > area["limit"][1]:
                        end = area["limit"][0]
                ...
                if len(tupdateOStream):
                    updateStream += [p, t, PARAMETER.ONSET,
                        len(tupdateOStream) / 2] + tupdateOStream

                if len(tupdateDStream):
                    updateStream += [p, t, PARAMETER.DURATION,
                        len(tupdateDStream) / 2] + tupdateDStream
    ...
    def addListener(self, listener, parasite=None, page=False, note=False):
        ...
        if page:
            self.pageListeners.append(listener)

        if note:
            self.noteListeners.append(listener)
        ...
        for p in self.tune:
            for t in range(Config.NUMBER_OF_TRACKS):
                ...
                for n in self.noteD[p][t].keys():
                    ...
                    # give parasites the option of returning something other than themselves
                    self.parasiteD[p][t][listener][n] = parasite.attach()
                    self.parasiteS[p][t][listener].insert(
                        self.noteS[p][t].index(self.noteD[p][t][n]), parasite.attach())
