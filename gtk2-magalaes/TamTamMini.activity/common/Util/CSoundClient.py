...
from common.Util.Clooper import sc_initialize
from common.Util.Clooper import sc_destroy
from common.Util.Clooper import sc_loop_new
from common.Util.Clooper import sc_start
from common.Util.Clooper import sc_stop
from common.Util.Clooper import sc_setChannel
from common.Util.Clooper import sc_inputMessage
from common.Util.Clooper import sc_setTickDuration
from common.Util.Clooper import sc_loop_playing
from common.Util.Clooper import sc_loop_delete
from common.Util.Clooper import sc_scoreEvent
from common.Util.Clooper import sc_loop_updateEvent
from common.Util.Clooper import sc_loop_addScoreEvent
from common.Util.Clooper import sc_loop_setNumTicks
from common.Util.Clooper import sc_loop_getTickf
from common.Util.Clooper import sc_loop_setTickf
from common.Util.Clooper import sc_loop_delScoreEvent
...
class _CSoundClientPlugin:
    ...
    def __init__(self):
        ...
        # loopNumTicks, while I wait for james to implement it properly
    ...
    def load_mic_instrument(self, inst):
        ...
        sc_inputMessage(
            Config.CSOUND_LOAD_INSTRUMENT % (instrumentId, fileName))
    ...
    def load_instruments(self):
        ...
        for instrumentSoundFile in self.instrumentDB.instNamed.keys():
            if instrumentSoundFile[0:3] == 'mic' or \
                ...
                self.instrumentDB.instNamed[instrumentSoundFile].category == 'mysounds':
    ...
    def setTempo(self, t):
        if (Config.DEBUG > 3):
            print 'INFO: loop tempo: %f -> %f' % (
                t, 60.0 / (Config.TICKS_PER_BEAT * t))
    ...
    def loopUpdate(self, note, parameter, value, cmd, loopId=_loop_default):
        ...
        if (parameter == NoteDB.PARAMETER.ONSET):
            ...
            if self.instrumentDB.instId[note.cs.instrumentId].kit != None:
                ...
                sc_loop_updateEvent(
                    loopId, (page<<16)+id, 0, (
                    csoundInstId + instrument_id_offset) + note.track * 0.01, -1)
            ...
            if (Config.DEBUG > 2):
                print 'INFO: updating instrument', (
                page<<16)+id, instrument.name, csoundInstId
                
            sc_loop_updateEvent(loopId, (page<<16)+id, 0, (
                csoundInstId + (track+1) + instrument_id_offset) + note.track * 0.01, cmd)
            sc_loop_updateEvent(loopId, (page<<16)+id, 7, csoundTable, -1)
    ...
    def loopPlay(self, dbnote, active,
        storage=_new_note_array(), loopId=_loop_default):
            
        qid = (dbnote.page << 16) + dbnote.id
        
        sc_loop_addScoreEvent(
            loopId, qid, 1, active, 'i',
    ...
    def _csnote_to_array1(self, storage, onset,
        pitch, amplitude, pan, duration,
        trackId, attack, decay, reverbSend,
        filterType, filterCutoff, tied,
        instrumentId, mode, instrumentId2 = -1):
