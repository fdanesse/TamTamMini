...
class ScrolledBox(gtk.EventBox):
    ...
    def __init__(self, orientation,
        ...
        if orientation == gtk.ORIENTATION_HORIZONTAL:
            ...
            self._scrolled.set_policy(
                gtk.POLICY_NEVER, scroll_policy)
            
        self._scrolled.connect('scroll-event',
            self._scroll_event_cb)
        ...
        self._adj.connect('changed',
            self._scroll_changed_cb)
            
        self._adj.connect('value-changed',
            self._scroll_changed_cb)
    ...
    def set_viewport(self, widget):
        ...
        if self._arrows_policy == gtk.POLICY_AUTOMATIC:
            self._aviewport_sig = self._viewport.connect(
            'size-allocate', self._viewport_allocate_cb)
    ...
    def _scroll_cb(self, widget, direction):
        ...
        if direction in (gtk.gdk.SCROLL_LEFT, gtk.gdk.SCROLL_UP):
            val = max(self._adj.get_property('lower'),
                self._adj.get_value() - self._adj.get_property(
                'page_increment'))
