...
from common.Generation.Drunk import Loopseg
from common.Generation.Drunk import Repeter
from common.Generation.Drunk import DroneAndJump
from common.Generation.Drunk import Drunk
...
class Loop():
    ...
    def precompose(self, maxbeat):
        ...
        def makePitchSequence(length, pitchRegularity,
            step, table_pitch, pitchMethod):
        ...
        def makeRythmSequence(barLength, density, regularity):
            ...
            for i in listLen:
                if self.count == 0:
                    currentOnsetValue = onsetValue + (
                        randInt(0, onsetDeviation) - (onsetDeviation / 2))
...
############ begin generate #####################
        ...
        for key in range(numKeys):
            ...
            for beat in range(2, maxbeat + 1):
                ...
                rythmSequence = makeRythmSequence(
                    barLength, density, regularity)
                    
                pitchSequence = makePitchSequence(
                    len(rythmSequence), pitchRegularity,
                    step, table_pitch, pitchMethod)
                ...
                for l in loopList:
                    f.write(
                        'note_add %s 1 0 %s %s %s 0.5 %s 0 1 0.005 0.098 0.1 0 1000 0 edit\n' % (
